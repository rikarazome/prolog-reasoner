...
└───────────────┬───────────────────────┘
                │ MCP Protocol (stdio)
┌───────────────▼───────────────────────┐
│  MCP Layer (server.py)                │  ← 薄いラッパー
├───────────────────────────────────────┤
│  Core Layer (reasoner.py)             │
│  ┌──────────┐  ┌──────────┐          │
│  │Translator│  │ Executor │          │
│  │(NL→PL+  │  │(PL実行)  │          │
│  │ 自己修正) │  │          │          │
...
├───────┼──────────────┼────────────────┤
│  ┌────▼─────┐  ┌─────▼────────────┐  │
│  │LLM Client│  │Prolog Subprocess │  │
│  │(直接API) │  │(SWI-Prolog)      │  │
│  └──────────┘  └──────────────────┘  │
└───────────────────────────────────────┘
```

**ファイルI/O（Prolog中間表現の保存）はこのシステムの責務ではない。** translate_to_prologが返すprolog_codeをファイルに保存するのは、MCPクライアント（LLM）側のツール（Write等）で行う。prolog-reasonerはコード文字列の入出力のみ担当する。
...
v1では`reason`, `execute_prolog`, `generate_prolog`の3ツールだったが、**2ツールに削減**する。

**根拠**: `reason`は`generate_prolog`+`execute_prolog`の合成にすぎない。LLM自身がツールを組み合わせてワークフローを構築できるため、合成ツールは不要。Unix哲学（小さく組み合わせ可能なツール）に従う。

### Tool 1: `translate_to_prolog`
...
async def translate_to_prolog(
    ...
    """
    自然言語をPrologコードに変換する。
    構文検証のためにSWI-Prologを使用するが、クエリの推論実行は行わない。
    生成されたコードを確認・修正してからexecute_prologで実行できる。

    Args:
        query: 自然言語の論理的質問や事実の記述
        context: 追加の前提条件や事実（自然言語）
        max_corrections: Prolog構文エラー時の自動修正回数（0で無効化）
    """
```
...
```json
{
    ...
    "suggested_query": "mortal(socrates)",
    "metadata": {
        "correction_iterations": 0,
        "translation_time_ms": 1520
    }
}
```

### Tool 2: `execute_prolog`
...
async def execute_prolog(
    ...
    translate_to_prologで生成したコードや、手動で書いたコードを実行できる。
...
def create_reasoner() -> PrologReasoner:
    settings = Settings()                # 環境変数から読み込み
    settings.validate_swipl()            # SWI-Prolog存在確認（起動時に1回）
...
reasoner = create_reasoner()  # モジュールレベルで1回
```

server.pyがモジュールロード時に`create_reasoner()`を呼び、以降の全リクエストで同一インスタンスを使用する。ライブラリとしてスタンドアロン利用する場合も同じパターン。

**FastMCPインスタンスとエントリポイント:**
...
# 2. translate_to_prolog
# 入力: "ソクラテスは人間。人間は死すべきもの。ソクラテスは死すべきものか？"
# 期待: success=true, prolog_codeにhuman(socrates)とmortal/1のルール

# 3. execute_prolog
# 入力: 上で生成されたprolog_code + query="mortal(socrates)"
# 期待: output="mortal(socrates)\n" (クエリラッパーがwrite_canonical(mortal(socrates))を出力)

# 4. execute_prolog（変数ありクエリ）
# 入力: 同じprolog_code + query="mortal(X)"
# 期待: output="mortal(socrates)\n"

# 5. 修正サイクル
# 生成されたPrologのルールを意図的に変更
# → execute_prolog → outputが変わることを確認
```
